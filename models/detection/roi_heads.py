...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(self,
        ...
        print("Feature Vecs:", feature_vectors[:2])
        # feature_vectors_repeated = feature_vectors.repeat(4, 1)
        ...
        for index, (boxes, scores, image_shape) in enumerate(zip(pred_boxes_list, pred_scores_list, image_shapes)):
            ...
            print("Checkpoint -1:", "Boxes Shape:", boxes.shape, "Scores Shape:", scores.shape)
            print("Before Transformation:", "Box 0:", boxes[0])
            # batch everything, by making every class prediction be a separate instance
            boxes = boxes.reshape(-1, 4)
            scores = scores.reshape(-1)
            labels = labels.reshape(-1)

            print("Checkpoint 0:", "Boxes Shape:", boxes.shape)
    ...
    def forward(self,
        ...
        if self.training:
            ...
            boxes, scores, labels, feature_vectors = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes, feature_vectors=feature_vectors)
